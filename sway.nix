...
# Take care to start the correct target as described by the sway project wiki.
...

{
  ...
  environment.systemPackages = with pkgs; [
    i3pystatus (python39.withPackages(ps: with ps; [ i3pystatus keyring ]))
    qt5.qtwayland
  ];
  ...
  programs.sway = {
    enable = true;
    extraPackages = with pkgs; [ wofi swaylock xwayland
      swayidle
      grim
      slurp
      swaybg
      wl-clipboard
      wf-recorder
      (python39.withPackages(ps: with ps; [ i3pystatus keyring ]))
    ];
